...
with open(dataset_path + 'AD.txt', encoding='utf-8') as f:
    for line in f:
        junk_list.append(line.split('\n')[0])  # 添加

with open(dataset_path + 'Vulgrs.txt', encoding='utf-8') as f:
    for line in f:
        junk_list.append(line.split('\n')[0])  # 添加

with open(dataset_path + 'corpus_final.txt', encoding='utf-8') as f:  # 语料
    for line in f:
        randomNum = random.randint(1, 100)  # 1-100随机抽
        if randomNum <= 70:  # 90是训练集，10是测试集
            weibo.append(line)
        if randomNum >= 70:
            test.append(line)
# 乱序
...
# print(y_train)
x_junk = np.zeros((length, 32, 256))  # 32是同一条微博下带的最多的垃圾词有32条
# 三个三维数组
counttt = 0
# 训练集的处理
    ...
    for j in range(L - 1):
        ...
        if junk_list.count(words[j]) != 0:  # 垃圾词表 中 word[j]出现的次数 只要出现一次即可 word[j] 是第i条的第j个词
            try:
                ...
                #if len(weibo[i])>6 and weibo[i].find('免税店')!=-1:
                # if weibo[i].find('人肉') != -1 and weibo[i].find('免税店') != -1:
                #     tmpJunk = 0
                #     counttt=counttt+1
                #     print('counttt',counttt)
                #     print('weibo[i]',len(weibo[i]),weibo[i])
                #     #print('words',words)
...
test_junk = np.zeros((test_length, 32, 256))
...
#在第一个dropout层和第二个mutiply层之后的东西 Tensor层  矩阵一行求和
def foo(x):
    #print('x',x)
    #print(K.sum(x, axis=1))
...
junk = Input(shape=(32, input_size), name='junk_input')  # 垃圾词的shape 模型的另一端
...
#CRF
#lstm_context = CRF(2,sparse_target=True)(lstm_context)
...
#加一层CRF
#lstm_final_representation = CRF(units=unit,sparse_target=True)(lstm_final_representation)
...
print('training---------------------------')
#metrics = Metrics()
x_train_final = [x_train,x_junk]
x_test_final = [x_test,test_junk]
model.fit(x_train_final, y_train, epochs=1, batch_size=16, validation_split=0.1)  # fit开始训练 #batch 几个一批
for layer in model.layers:
    weights = layer.get_weights()  # list of numpy array
    with open('song.txt', 'a', encoding='utf-8') as f:
        print(f.write(str(weights)+'\n\n'))
...
#model.fit([x_train, x_junk], y_train, epochs=7, batch_size=32,validation_split=0.1,
          #validation_data=([x_test, test_junk], y_test),
          #callbacks=[metrics])
print('testing-----------------------------')
score = model.evaluate(x_test_final, y_test)  # 开始评估
print(score)
#以上是自带的评估
...
for i in range(total):
    ...
    #tot = tot + predictions[i];
    if predictions[i] >= 0.5:
        predictions_final[i] = 1
    elif predictions[i] < 0.5:
        predictions_final[i] = 0
    #print(predictions_final[i], y_test[i])

#print(predictions_final)
#传一个预测矩阵和一个实际矩阵就行
print('f1',metrics.f1_score(predictions_final, y_test, average='macro'))
print('pre',metrics.precision_score(predictions_final, y_test, average='macro'))
print('rec',metrics.recall_score(predictions_final, y_test, average='macro'))